...
public abstract class AbstractConnectionBean extends com.antlersoft.android.dbimpl.IdImplementationBase implements Connection {
    ...
    public static String retrieveConfigFileFromIntent(Intent i, String filesDir, Context context, Object waitOn) throws GettingConnectionSettingsException {
        ...
        Log.d(TAG, "=== retrieveConfigFileFromIntent ===");
        Log.d(TAG, "Got intent: " + i);
        Log.d(TAG, "data: " + data);
        Log.d(TAG, "tempConfigFile: " + tempConfigFile);
        ...
        if (data != null) {
            ...
            if (dataString.startsWith("http")) {
                Log.d(TAG, "Using http download path");
                ...
                Log.d(TAG, "Intent is with content scheme, copying to temp file...");
                ...
                try {
                    ...
                    Log.d(TAG, "Copied content to: " + configFileName);
                } catch (IOException e) {
    ...
    public static Connection getRemoteConnectionSettings(Intent i, Context context, boolean masterPasswordEnabled) throws GettingConnectionSettingsException {
        ...
        Log.d(TAG, "=== getRemoteConnectionSettings ===");
        Log.d(TAG, "isOpaque=" + Utils.isOpaque(context));
        Log.d(TAG, "configFileName=" + configFileName);
        Log.d(TAG, "intent type=" + i.getType());
        Log.d(TAG, "intent data=" + i.getData());
        if (Utils.isOpaque(context)) {
            connection = getOpaqueConnection(i, context, configFileName);
            Log.d(TAG, "getOpaqueConnection returned: " + connection);
            // If opaque connection is null (no extra found), fall back to URI-based connection
            if (connection == null && !Utils.isNullOrEmptry(i.getType())) {
                Log.d(TAG, "Opaque connection is null, falling back to MIME type based connection");
                connection = tryGetConnectionFromUri(i, context, masterPasswordEnabled);
                Log.d(TAG, "tryGetConnectionFromUri returned: " + connection);
            }
        } else if (configFileName != null) {
            // 允许所有版本（包括免费版）打开 .vv 文件
            Log.d(TAG, "Using configFileName path");
            connection = tryGetConnectionFromUri(i, context, masterPasswordEnabled);
            Log.d(TAG, "tryGetConnectionFromUri returned: " + connection);
        } else {
            Log.d(TAG, "Using direct URI path");
            connection = tryGetConnectionFromUri(i, context, masterPasswordEnabled);
            Log.d(TAG, "tryGetConnectionFromUri returned: " + connection);
        }
        if (connection != null) {
        ...
        Log.d(TAG, "Final connection: " + connection);
        return connection;
    }
    ...
    private static Connection getOpaqueConnection(Intent i, Context context, String configFileName) {
        Connection connection;
        Log.d(TAG, "=== getOpaqueConnection ===");
        Log.d(TAG, "configFileName is null: " + (configFileName == null));

        // Check for vv_file_path extra first (passed directly from Flutter)
        String vvFilePath = i.getStringExtra("vv_file_path");
        if (vvFilePath != null) {
            Log.d(TAG, "Found vv_file_path extra: " + vvFilePath);
            try {
                connection = new ConnectionSettings(RemoteClientLibConstants.DEFAULT_SETTINGS_FILE);
                // Copy the .vv file content to the default settings file location
                java.io.File vvFile = new java.io.File(vvFilePath);
                if (vvFile.exists()) {
                    java.io.File destFile = new java.io.File(context.getFilesDir(), RemoteClientLibConstants.DEFAULT_SETTINGS_FILE);
                    java.io.FileInputStream fis = new java.io.FileInputStream(vvFile);
                    ...
                    fis.close();
                    fos.close();
                    Log.d(TAG, "Copied .vv file to: " + destFile.getAbsolutePath());
                    connection.load(context);
                    Log.d(TAG, "ConnectionSettings loaded successfully from vv_file_path");
                    // Set connectionConfigFile so startConnectionDirectlyOrFromFile() will call startFromVvFile()
                    connection.setConnectionConfigFile(destFile.getAbsolutePath());
                    return connection;
                }
            } catch (Exception e) {
                Log.e(TAG, "Failed to load from vv_file_path: " + e.getMessage());
            }
        }

        if (configFileName == null) {
            Log.d(TAG, "Initializing session from connection settings (extra).");
            connection = (ConnectionSettings) i.getSerializableExtra(Constants.opaqueConnectionSettingsClassPath);
            Log.d(TAG, "Connection from extra: " + connection);
            // If no config from extra and we have a MIME type, try MIME type based connection
            if (connection == null && !Utils.isNullOrEmptry(i.getType())) {
                Log.d(TAG, "No connection from extra, creating ConnectionBean for MIME type");
                connection = new ConnectionBean(context);
                Log.d(TAG, "Created ConnectionBean: " + connection);
            }
        } else {
            // When we have a configFileName, we might be dealing with a .vv file
            Log.d(TAG, "Loading from configFileName: " + configFileName);
            // Try to create a ConnectionSettings and load from file
            try {
                connection = new ConnectionSettings(RemoteClientLibConstants.DEFAULT_SETTINGS_FILE);
                connection.load(context);
                Log.d(TAG, "ConnectionSettings loaded successfully: " + connection);
            } catch (Exception e) {
                Log.e(TAG, "Failed to load ConnectionSettings: " + e.getMessage());
                // Fall back to ConnectionBean if load fails
                connection = new ConnectionBean(context);
                Log.d(TAG, "Fell back to ConnectionBean: " + connection);
            }
        }
        return connection;
    }
    ...
    private static Connection getConnection(Intent i, Context context, boolean masterPasswordEnabled) throws GettingConnectionSettingsException {
        ...
        boolean hasMimeType = !Utils.isNullOrEmptry(i.getType());
        Log.d(TAG, "=== getConnection ===");
        Log.d(TAG, "isSupportedScheme=" + isSupportedScheme + " (data=" + data + ")");
        Log.d(TAG, "hasMimeType=" + hasMimeType + " (type=" + i.getType() + ")");

        // If we have a MIME type (like application/x-virt-viewer), prioritize MIME-based connection
        // This ensures .vv files are properly loaded with all parameters including CA certificates
        if (hasMimeType) {
            Log.i(TAG, "getConnection - has MIME type, creating ConnectionBean");
            connection = new ConnectionBean(context);
    ...
    private static boolean isSupportedScheme(Uri data) {
        ...
        if (data != null) {
            ...
            // Also recognize spice+https and spice+http as valid SPICE schemes
            isSupportedScheme = "rdp".equals(s) || "spice".equals(s) || "vnc".equals(s) ||
                    "spice+https".equals(s) || "spice+http".equals(s);
        }
