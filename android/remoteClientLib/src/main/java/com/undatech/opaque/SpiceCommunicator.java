...
public class SpiceCommunicator extends RfbConnectable {
    ...
    private static boolean isGStreamerInitialized = false;
    ...
    private void unregisterReceivers(Context context) {
        if (usbEnabled) {
            try {
                context.unregisterReceiver(usbPermissionRequestedReceiver);
            } catch (IllegalArgumentException e) {
                // Receiver not registered
            }
            try {
                context.unregisterReceiver(usbStateChangedReceiver);
            } catch (IllegalArgumentException e) {
                // Receiver not registered
            }
        }
    ...
    public void close() {
        synchronized (SpiceCommunicator.class) {
            if (myself == null) {
                Log.d(TAG, "close() called but myself is already null, skipping");
                return;
            }
            // Always disconnect at native level to ensure clean state
            SpiceClientDisconnect();
            disconnect();
            if (savedContext != null) {
                unregisterReceivers(savedContext);
                savedContext = null;  // 清除 context 引用，防止内存泄漏
            }
            myself = null;
        }
    }
