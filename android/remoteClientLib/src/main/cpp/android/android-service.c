...
gboolean getJvmAndMethodReferences(JNIEnv *env) {
    ...
    jni_remote_clipboard_changed = (*env)->GetStaticMethodID(env, jni_connector_class, "OnRemoteClipboardChanged",
                                                             "(Ljava/lang/String;)V");
    jni_add_vm = (*env)->GetStaticMethodID(env, jni_connector_class, "AddVm", "(Ljava/lang/String;)V");
    jni_launch_vnc_viewer = (*env)->GetStaticMethodID(env, jni_connector_class, "LaunchVncViewer",
...
int spiceClientConnectVv(VirtViewerFile *vv_file, const gboolean sound) {
    ...
    if (global_conn != NULL) {
        __android_log_write(ANDROID_LOG_INFO, "spiceClientConnectVv", "Existing connection found, disconnecting first.");
        connection_disconnect(global_conn);
        // Note: disconnect is async, main loop will handle cleanup
    }
...
void sendMessage(
    ...
    // Use cached global class reference instead of FindClass every time
    if (jni_connector_class != NULL) {
        jmethodID sendMessage = (*env)->GetStaticMethodID(env, jni_connector_class, "sendMessageWithText", "(ILjava/lang/String;)V");
        jstring messageText = (*env)->NewStringUTF(env, message_text);
        (*env)->CallStaticVoidMethod(env, jni_connector_class, sendMessage, messageID, messageText);
    }
...
static void send_vm_names_to_ui(JNIEnv *env, OvirtCollection *vms) {
    ...
    while (g_hash_table_iter_next(&iter, &vmname, &vm)) {
        ...
        if (vmName != NULL && jni_add_vm != NULL) {
            // Call back into SpiceCommunicator to add this VM to the list
            (*env)->CallStaticVoidMethod(env, jni_connector_class, jni_add_vm, vmName);
        }
    }
}
...
Java_com_undatech_opaque_SpiceCommunicator_StartSessionFromVvFile(
    ...
    // Don't store env globally - let sendMessage attach current thread
...
int startVncSession(JNIEnv *env, const gchar *gport, const gchar *ghost, const gchar *ticket) {
    ...
    if (vncAddress != NULL && vncPort != NULL && vncPassword != NULL && jni_launch_vnc_viewer != NULL) {
        // Call back into SpiceCommunicator to start external app
        (*env)->CallStaticVoidMethod(env, jni_connector_class, jni_launch_vnc_viewer, vncAddress, vncPort, vncPassword);
    } else {
